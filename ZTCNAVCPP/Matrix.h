...
public:
	...
	/// 获取矩阵对应的二维数组
	/// </summary>
	inline constexpr std::vector<std::vector<double>> ToArray() const noexcept
	{
		return _elements;
	}
	...
	/// 二维矩阵的行数
	/// </summary>
	inline constexpr int RowsCount() const noexcept
	{
		return _rows;
	}
	...
	/// 二维矩阵的列数
	/// </summary>
	inline constexpr int ColumnsCount() const noexcept
	{
		return _columns;
	}
	...
	/// 返回第i+1行可变引用，不做下标检查
	/// </summary>
	inline constexpr std::vector<double>& operator [](int i)
	{
		return _elements[i];
	}
	...
	/// 返回第i+1行常量引用，不做下标检查
	/// </summary>
	inline constexpr const std::vector<double>& operator [](int i) const
	{
		return _elements[i];
	}
	...
	/// <param name="j"></param>
	/// <returns></returns>
	inline constexpr const double& operator()(int i, int j) const
	{
		return _elements.at(i).at(j);
	}
